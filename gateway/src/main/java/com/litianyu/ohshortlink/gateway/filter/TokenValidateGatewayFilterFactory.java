package com.litianyu.ohshortlink.gateway.filter;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import com.litianyu.ohshortlink.gateway.config.Config;
import com.litianyu.ohshortlink.gateway.dto.GatewayErrorResult;
...
/**
 * SpringCloud Gateway Token 拦截器（鉴权过滤器）
 */
@Component
public class TokenValidateGatewayFilterFactory extends AbstractGatewayFilterFactory<Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) { // 不在白名单，验证后放行
                String username = request.getHeaders().getFirst("username");
                String token = request.getHeaders().getFirst("token");
                Object userInfo;
                // 通过 username 和 登录时发放的 token 验证登陆状态
                if (StringUtils.hasText(username) && StringUtils.hasText(token) && (userInfo = stringRedisTemplate.opsForHash().get("short-link:login:" + username, token)) != null) {
                    ...
                    return chain.filter(exchange.mutate().request(builder.build()).build()); // 通过登陆验证，放行
                }
                // 没有通过登陆验证，直接返回失败
